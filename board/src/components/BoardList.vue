<template>
  <div>
          <p class="addnew"><button class="btn btn-primary" v-on:click="addCreateForm">새로운 연락처 추가</button></p>
      <div id="example">
        <table id="list" class="table table-striped table-bordered table-hover">
            ...
            <tbody id="contacts" >
                <tr v-for="(item, index) in list.contacts" :key="index">
                    <td>{{item.name}}</td>
                    <td>{{item.tel}}</td>
                    <td>{{item.address}}</td>
                    <td><img class="thumbnail" v-bind:src="item.photo" v-on:click="editPhoto(item)" /></td>
                    <td>
                        <button class="btn btn-primary" v-on:click="updateBtn(item)" >편집</button>
                        <button class="btn btn-primary" v-on:click="removeBtn(item.no)">삭제</button>
                    </td>
                </tr>
            </tbody>
        </table>
...
export default {
    ...
    computed : {
        totalpage() {
            return Math.floor((this.list.totalcount - 1) / this.list.pagesize) + 1;
        }
    },
    methods: {
        addCreateForm(){
            eventBus.$emit("addCreateForm");
        },
        pageChanged(page){
            eventBus.$emit("pageChanged", page);
        },
        removeBtn(no){
            eventBus.$emit("removeItem",no);
        },
        updateBtn(item){
            eventBus.$emit("editForm",item);
        },
        editPhoto(item){
            eventBus.$emit("editPhoto",item);
        }
    },
}
</script>
